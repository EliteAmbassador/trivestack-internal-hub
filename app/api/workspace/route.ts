...
export async function GET(request: Request) {
  try {
    ...
    const reports = await db.prepare("SELECT r.*, u.full_name AS owner_name, u.team AS owner_team, p.name AS project_name FROM reports r JOIN users u ON u.id = r.user_id JOIN projects p ON p.id = r.project_id ORDER BY COALESCE(r.submitted_at, r.created_at) DESC").all<ReportRow>();
